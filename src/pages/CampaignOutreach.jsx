...
import {
  Send, Mail, MessageCircle, Download, Search, 
  Trash2, Sparkles, Target, Users, Loader,
  CheckCircle
} from 'lucide-react';
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white flex items-center gap-3">
              ...
              Campaign Outreach
              <span className="text-sm bg-gradient-to-r from-orange-500 to-red-500 px-3 py-1 rounded-full text-white text-xs font-medium animate-pulse">LIVE</span>
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-purple-500/20">
            <h3 className="text-white font-semibold flex items-center gap-2 mb-4">
              <Target size={20} className="text-purple-400" /> Campaign Setup
            </h3>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`p-3 rounded-lg flex flex-col items-center gap-2 transition-all ${
                    activeChannel === 'whatsapp' ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg' : 'bg-slate-700/50 text-gray-400 hover:bg-slate-600/50'
                  }`}
                >
                ...
                <button
                  ...
                  className={`p-3 rounded-lg flex flex-col items-center gap-2 transition-all ${
                    activeChannel === 'email' ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white shadow-lg' : 'bg-slate-700/50 text-gray-400 hover:bg-slate-600/50'
                  }`}
                >
              ...
              />
              <div className="text-xs text-gray-500 mt-1">Variables: {'{{name}}'}, {'{{company}}'}</div>
              ...
              >
                {sending ? <><Loader size={18} className="animate-spin" /> Sending...</> : <><Send size={18} /> Send to {selectedLeads.length} Leads</>}
              </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-purple-500/20 overflow-hidden">
                        ...
                        <tr key={lead._id} className="border-t border-slate-700 hover:bg-slate-700/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => { if (window.confirm('Delete this lead?')) deleteLead(lead._id); }}
              ...
              <div className="p-4 border-t border-slate-700 flex justify-between items-center">
                <span className="text-gray-400 text-sm">{filteredLeads.length} leads ready for campaign</span>
              </div>
            </div>
          </div>
        </div>
