...
import { 
  Users, Brain, Zap, Target, Search, Download, Trash2, 
  Eye, Edit, ArrowUp, ArrowDown, Activity 
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white flex items-center gap-3">
              ...
              AI Dashboard
              <span className="text-sm bg-gradient-to-r from-purple-500 to-pink-500 px-3 py-1 rounded-full text-white text-xs font-medium animate-glow">LIVE</span>
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat, index) => (
            <div key={index} className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-purple-500/20 hover:border-purple-500/50 transition-all card-hover">
              ...
              <div className="flex items-center gap-2 mt-3">
                {stat.up ? <ArrowUp size={16} className="text-green-400" /> : <ArrowDown size={16} className="text-red-400" />}
                <span className={stat.up ? 'text-green-400 text-sm' : 'text-red-400 text-sm'}>{stat.change}</span>
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-purple-500/20 overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody>
                {filteredLeads.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="px-4 py-8 text-center text-gray-400">No leads found</td>
                  </tr>
                ) : (
          ...
          <div className="p-4 border-t border-slate-700 flex justify-between items-center">
            <span className="text-gray-400 text-sm">Showing {filteredLeads.length} of {leads.length} leads</span>
          </div>
        </div>
      </div>
    </div>
